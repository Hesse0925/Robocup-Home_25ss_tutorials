...
public:
            ...
            ros::NodeHandle&            controller_nh)
  {
    ...
    nh_ = controller_nh;  // or: nh_ = ros::NodeHandle("~");
    debug_pub_ = nh_.advertise<std_msgs::Float64>("debug", 10);

    ROS_INFO_STREAM("NewHeadController initialized successfully.");
    return true;
  }
  ...
  void update(const ros::Time& time, const ros::Duration& period)
  {
    ...
    //{
      cont += 0.01;
      double pos = -0.2;
      double cmd = 0.2*sin(cont) + pos;

      joints_[0].setCommand(cmd);

      ROS_WARN_STREAM("cmd = " << cmd);
      ROS_WARN_STREAM("joint_pos = " << joints_[0].getPosition());
      ROS_INFO_STREAM("time = " << time.toSec());

      // Optional debug topic publish
...
private:
  ...
  double cont;
  ros::Publisher debug_pub_;           // ROS publisher for debug info
  std_msgs::Float64 debug_msg_;        // Reusable message
  ros::NodeHandle nh_;                 // NodeHandle to create publisher
