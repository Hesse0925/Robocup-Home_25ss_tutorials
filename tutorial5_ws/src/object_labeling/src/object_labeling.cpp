...
bool ObjectLabeling::initalize(ros::NodeHandle& nh)
{
  ...
  no_object_pub_ = nh.advertise<std_msgs::Empty>("/no_objects_detected", 1);
  no_cup_pub_ = nh.advertise<std_msgs::Empty>("/no_cup_detected", 1);
...
bool ObjectLabeling::labelObjects(CloudPtr& input, CloudPtrl& output)
{
  ...
  if (cluster_indices.empty()) {
    ROS_WARN("No clusters found — likely no object on table.");

    // Optional: publish empty message to notify others
    std_msgs::Empty msg;
    no_object_pub_.publish(msg);
  ...
  for (const auto& class_name : assigned_classes) {
    if (class_name == "cup") {
      cup_found = true;
      break;
    }
  ...
  if (!cup_found) {
    ROS_WARN("No cup detected among labeled objects.");

    std_msgs::Empty msg;
    no_cup_pub_.publish(msg);
  }
