...
bool PlaneSegmentation::initalize(ros::NodeHandle& nh)
{
  ...
  plane_cloud_pub_ = nh.advertise<sensor_msgs::PointCloud2>("/segmented_table", 1);
  //#>>>>TODO: advertise the pointcloud for the remaining points (objects)
  objects_cloud_pub_ = nh.advertise<sensor_msgs::PointCloud2>("/segmented_objects", 1);
  // initialize centriod publisher
  object_centroid_pub_ = nh.advertise<geometry_msgs::PointStamped>("/object_centroid", 1);
...
void PlaneSegmentation::update(const ros::Time& time)
{
  ...
  if(is_cloud_updated_)
  {
    ...
    pcl::toROSMsg(*plane_cloud_, table_msg);
    table_msg.header.frame_id = base_frame_;
    table_msg.header.stamp = time;
    plane_cloud_pub_.publish(table_msg);
  ...
  // Compute the centroid of the objects cloud
  if (!objects_cloud_->empty())
  {
    Eigen::Vector3f centroid(0.0, 0.0, 0.0);
    for (const auto& pt : *objects_cloud_)
    {
      centroid += Eigen::Vector3f(pt.x, pt.y, pt.z);
    }
    centroid /= static_cast<float>(objects_cloud_->size());

    // Create and publish a PointStamped message
    geometry_msgs::PointStamped centroid_msg;
    centroid_msg.header.frame_id = base_frame_;
  ...
  {
    ROS_WARN_THROTTLE(5.0, "Object cloud is empty. Cannot compute centroid.");
  }
...
bool PlaneSegmentation::preProcessCloud(CloudPtr& input, CloudPtr& output)
{
  ...
  ROS_INFO_STREAM("Input pointcloud frame: " << input->header.frame_id);
  ...
  pass.setFilterLimits(0.01, 2.0);  // keep points between 1cm and 2m in height
  pass.filter(*output);
  return true;
}
...
bool PlaneSegmentation::segmentCloud(CloudPtr& input, CloudPtr& plane_cloud, CloudPtr& objects_cloud)
{
  ...
  seg.setDistanceThreshold(0.01);  // distance threshold in meters
  ...
  {
    ROS_WARN("No planar model found in point cloud.");
    return false;
  }

  //#>>>>TODO: save inliers in plane_cloud 
  //#>>>>Note: These should be points that belong to the table 
  ...
  // Next, we further refine the objects_cloud by transforming it into the coordinate frame
  ...
  //#>>>>TODO: to the origin from the plane coefficients.
  //#>>>>Note: As always we use Eigen to represent vectors and matrices 
  ...
  //#>>>>TODO: Build the Rotation (Quaternion) from the table's normal vector n
  ...
  // // Add artificial Gaussian noise to simulate sensor noise
  // std::default_random_engine generator(std::random_device{}());
  // std::normal_distribution<float> dist(0.0f, 0.0001f); // mean = 0, stddev = 5mm

  // for (auto& pt : *objects_cloud) {
  //   pt.x += dist(generator);
  //   pt.y += dist(generator);
  //   pt.z += dist(generator);
  // }
  ...
  // pass.setInputCloud(transf_cloud);
  // pass.setFilterFieldName("z");
  // pass.setFilterLimits(0.01, 0.2); // keep points between 1cm and 20cm above the table
