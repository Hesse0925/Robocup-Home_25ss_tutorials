...
namespace tiago_localization
{
    ...
    bool Localizer::initialize(ros::NodeHandle& nh)
    {
        ...
        if(!ros::param::get(/*#>>>>TODO: PARAMETER NAME*/"spin_speed", speed))
            return false;
        if(!ros::param::get(/*#>>>>TODO: PARAMETER NAME*/"covariance_threshold", threshold))
            return false;
    ...
    void Localizer::poseCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msg) {
        ...
        ros::Duration interval = ros::Time::now() - msg->header.stamp;
        covariance_sum = 0;
        if (interval.toSec() <= 0.5/*#>>>>TODO: CHECK THE INTERVAL IS WITHIN 0.5 SECOND*/){
