...
public:
    ...
    void initialize()
    {
        arm_group_.setPlannerId("RRTstarConfigDefault");
        ...
        ros::NodeHandle nh;
        centroid_sub_ = nh.subscribe("/marker_3d_position_0", 1, &GraspController::centroidCallback, this);
        gripper_pub_ = nh.advertise<trajectory_msgs::JointTrajectory>("/parallel_gripper_controller/command", 10);

        ROS_INFO("GraspController initialized and waiting for red marker...");
    }
    ...
    void centroidCallback(const geometry_msgs::PointStamped::ConstPtr& msg)
    {
        ...
        try {
            ...
            stamped_in.header.stamp = ros::Time(0);  // ✅ 修复时间戳问题
