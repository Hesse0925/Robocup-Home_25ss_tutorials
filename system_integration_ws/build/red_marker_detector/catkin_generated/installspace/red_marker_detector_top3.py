...
class RedMarkerDetector:
    def __init__(self):
        ...
        self.max_marker_count = 3
        self.history_len = 5
        self.stability_threshold = 0.01  # 1cm

        # For filtering
        self.marker_history = [deque(maxlen=self.history_len) for _ in range(self.max_marker_count)]
        ...
        self.pixel_pubs = [
        rospy.Publisher(f'/marker_pixel_position_{i}', Int32MultiArray, queue_size=1)
        for i in range(self.max_marker_count)
]

        rospy.loginfo("Red marker detector with smoothing and stability check initialized.")
        rospy.spin()
    ...
    def try_detect_markers(self):
        ...
        for c in contours:
            ...
            marker_list.append((x, y, z, u, v))

        # Sort by x descending
        marker_list.sort(key=lambda pt: pt[0], reverse=True)
        marker_list = marker_list[:self.max_marker_count]

        self.clear_old_markers()

        for i, (x, y, z, u, v) in enumerate(marker_list):
            pos = np.array([x, y, z])
            self.marker_history[i].append(pos)

            self.pixel_pubs[i].publish(Int32MultiArray(data=[u, v]))
            
            if len(self.marker_history[i]) < self.history_len:
                continue  # 等待填满队列

            history = np.array(self.marker_history[i])
            mean = np.mean(history, axis=0)
            max_dev = np.max(np.linalg.norm(history - mean, axis=1))

            if max_dev < self.stability_threshold:
                # 稳定，发布均值点
                self.publish_filtered_point(i, *mean)
                self.publish_axes(*mean, base_id=i * 10)
                rospy.loginfo(f"[Stable Marker #{i}] x={mean[0]:.3f}, y={mean[1]:.3f}, z={mean[2]:.3f}")
            else:
                rospy.loginfo(f"[Unstable Marker #{i}] deviation={max_dev:.4f}")

    def publish_filtered_point(self, i, x, y, z):
