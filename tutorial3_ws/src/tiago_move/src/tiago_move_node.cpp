...
// typedef /*#>>>>TODO: ACTION CLIENT TYPE*/</*#>>>>TODO: ACTION NAME*/> MoveBaseClient;
  ...
  bool Controller::initialize(ros::NodeHandle& nh)
  {
    ...
    for (const auto& wp : {"A", "B", "C"}) {
      std::vector<double> goal_vec;
      for (int i = 0; i < 3; ++i) {
        goal_vec.push_back(static_cast<double>(waypoint_list[wp][i]));
      }
      nav_goals.push_back(createGoal(goal_vec));
    }
...
int main(int argc, char** argv){
  ...
  ros::NodeHandle nh;
  ros::Publisher arm_trigger_pub = nh.advertise<std_msgs::Empty>("/arm_trigger", 1);
  ...
  while (ros::ok())
  {
      ...
      if (controller.ac->getState() == actionlib::SimpleClientGoalState::SUCCEEDED) /*#>>>>TODO:Exercise3 Check if the state of this goal is SUCCEEDED use getState function of SimpleActionClient instace*/
      //#>>>> Hint: see https://docs.ros.org/en/diamondback/api/actionlib/html/classactionlib\_1\_1SimpleActionClient.html for the return type of getState function
      {
          ...
          {
              std_msgs::Empty msg;
              arm_trigger_pub.publish(msg);
              ROS_INFO("Published arm trigger signal.");
              ros::Duration(5.0).sleep(); // wait for arm to finish
          }
